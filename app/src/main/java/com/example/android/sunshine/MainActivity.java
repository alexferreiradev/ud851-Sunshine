...
public class MainActivity extends AppCompatActivity implements ForecastAdapterOnClickHandler, LoaderManager.LoaderCallbacks<String[]> {
    ...
    public static final String CAN_USE_CACHE_EXTRA_KEY = "can_use_cache_extra_key";
     ...
     */
    private void loadWeatherData(boolean canUseCache) {
        showWeatherDataView();

        String location = SunshinePreferences.getPreferredWeatherLocation(this);
//        new FetchWeatherTask().execute(location);
        Bundle bundle = new Bundle();
        bundle.putString(LOCATION_EXTRA_KEY, location);
        bundle.putBoolean(CAN_USE_CACHE_EXTRA_KEY, canUseCache);
        LoaderManager supportLoaderManager = getSupportLoaderManager();
        Loader<String> loader = supportLoaderManager.getLoader(FORECAST_LOADER_ID);
        if (loader != null) {
            supportLoaderManager.restartLoader(FORECAST_LOADER_ID, bundle, this);
        } else {
            supportLoaderManager.initLoader(FORECAST_LOADER_ID, bundle, MainActivity.this);
        }
    }
    ...
    // (4) When the load is finished, show either the data or an error message if there is no dat
    ...
    public Loader<String[]> onCreateLoader(int id, Bundle args) {
        return new AsyncTaskLoader<String[]>(this) {
            private String[] mForecastCache;

            @Override
            protected void onStartLoading() {
                super.onStartLoading();
                if (args.isEmpty()) {
                    mErrorMessageDisplay.setText("Não foi informado a localizaçao");
                    return ;
                }

                if (mForecastCache != null && args.getBoolean(CAN_USE_CACHE_EXTRA_KEY)) {
                    deliverResult(mForecastCache);
                } else {
            ...
            public String[] loadInBackground() {
                String location = args.getString(LOCATION_EXTRA_KEY);
